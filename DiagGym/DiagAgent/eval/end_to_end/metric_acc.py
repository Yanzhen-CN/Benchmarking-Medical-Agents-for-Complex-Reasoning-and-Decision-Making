...
def workflow(messages, model="gpt-4o", temperature=0.0, max_tokens=1024):
    """
    Call the OpenAI API to process messages.
...
def build_accuracy_fullchain_prompt(pred_diag, gt_diag):
    """
    Build prompt for evaluating diagnosis accuracy.
...
def evaluate_accuracy_multithread(json_filepath, save_filepath, max_workers=4, max_retry=5):
    """
    Evaluate accuracy for generated diagnoses.

    Args:
        json_filepath (str): Path to JSON file containing generated diagnoses.
    ...
    for item_idx, item in enumerate(json_data):
        ...
        last_msg = item['messages'][-1]['content']
        gt = item['ground_truth']

        # Only evaluate if the model claims enough info for diagnosis
        if 'The available information is sufficient to make a diagnosis.' not in last_msg:
            continue
...
if __name__ == "__main__":
    ...
    for filename in os.listdir('outputs'):
        if f'passrate_{filename}' not in exists_names and 'tmp' not in filename:
            filenames.append(filename)

    for filename in filenames:
        evaluate_accuracy_multithread(
            json_filepath=os.path.join('outputs', filename),
            save_filepath=os.path.join('metric_results', f'passrate_{filename}'),
            max_workers=64,
            max_retry=500
        )