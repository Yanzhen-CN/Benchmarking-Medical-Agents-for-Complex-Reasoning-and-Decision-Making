...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty="kl"):
    ...
    response_mask = data.batch["response_mask"]# attention_mask[:, -response_length:]
...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        if "custom_cls" in self.config.data and self.config.data.custom_cls.get("path", None) is not None:
            ...
            print(f"Using custom dataset class '{self.config.data.custom_cls.name}' from '{self.config.data.custom_cls.path}'")
        else:
            dataset_cls = RLHFDataset
            print("Using default RLHFDataset")
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "multi_modal_inputs" in test_batch.non_tensor_batch.keys():
                ...
                test_gen_batch = test_batch.pop(
                    batch_keys=["input_ids", "attention_mask", "position_ids"],
                    non_tensor_batch_keys=["raw_prompt_ids", "ehr_text",],
                )
